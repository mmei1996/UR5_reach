...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        self.obs_buf = np.zeros(core.combined_shape(size, 15), dtype=np.float32) # obs_dim + goal_dim
        self.obs2_buf = np.zeros(core.combined_shape(size, 15), dtype=np.float32)
...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    # direcly use observation as goal
        if mode == 'shaping':
            ...
            joint_velocity0 = abs(state[3])
            joint_velocity1 = abs(state[4])
            joint_velocity2 = abs(state[5])
            joint_velocity3 = abs(state[6])
            joint_velocity4 = abs(state[7]) 
            
            
            xx, yy, zz = state[9], state[10], state[11]
            dis = math.sqrt(xx**2+yy**2+zz**2)
            

            joint_velocity = max(joint_velocity0,joint_velocity1,joint_velocity2,joint_velocity3,joint_velocity4)
            #print(((1.5-dis)>0)* joint_velocity)
            costs = (goal_x - x)**2 + (goal_y - y)**2 + (goal_z - z)**2
            #reward1 = 0 if math.sqrt(costs) < tolerance and ((1.5-dis)>0)* joint_velocity<0.15 else -1
            #reward2 = 0 if ((1.5-dis)>0)* joint_velocity<0.3 else -1.5

            #reward= - 0.02*(math.sqrt(costs) > tolerance) - (((1.5-dis)>0)* joint_velocity>0.15)
            reward1 =  ((math.sqrt(costs)) < tolerance) - 2*(((1.5-dis)>0)* joint_velocity>0.3)
            
        return reward1

    def gene_new_sas(new_goals, transition):
        state, new_state = transition[0][:12], transition[3][:12]
        action = transition[1]
        ...
        if t >= update_after and t % update_every == 0:
            for i, transition in enumerate(episode_cache):
                    ...
                    for new_goal in new_goals:
                        reward = calcu_reward(new_goal, transition[0][:12], transition[1]) #new goal state action
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='DDPG_HER_V2')
